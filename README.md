# On Run Works

##  pipeline的泛化能力，以及composed pipeline的能力

### models list
    gpt： gpt oss, gpt 4o, gpt-5-nano,gpt-5-chat,gpt-5-mini, gpt-3.5-turbo-1106, gpt-3.5-turbo-0125

    claude: claude-3-7-sonnet-latest, claude-3-5-haiku-latest, claude-opus-4-0

    llama: TA/meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo, 
    TA/meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo, 
    TA/meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo, 
    TA/meta-llama/Llama-2-7b-chat-hf, 
    TA/meta-llama/Llama-2-13b-chat-hf, 

    Mistral: mistral-large|medium|small-latest

    gemini: gemini 2.5 flash|pro, gemini 2.0 flash, gemini 1.5 pro|flash

    qwen: TA/Qwen/Qwen3-235B-A22B-fp8-tput, TA/Qwen/Qwen2.5-7B-Instruct-Turbo, TA/Qwen/Qwen2.5-72B-Instruct-Turbo

