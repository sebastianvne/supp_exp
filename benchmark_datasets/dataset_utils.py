import pandas as pd
import os
from models.closesource import ClosesourceModel
...
def process_dataset(dataset_path:str, output_path:str) -> None:
    ...
    client = ClosesourceModel(model='gpt-4o',api_key=os.getenv('API_KEY'))
    for index, row in dataset.iterrows():
        prompt = row['prompt']
        dataset.at(index,'pipeline_category') = conclude_category(client,prompt)
        dataset.to_csv(output_path,index=True,encoding='utf-8')
    return dataset
